...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes docPillRecoil{0%,100%{transform:translateX(0) scale(1);box-shadow:none}6%{transform:translateX(-9px) scaleX(.955);box-shadow:-8px 0 0 rgba(194,65,12,.16)}16%{transform:translateX(4px) scaleX(1.018);box-shadow:0 0 0 4px rgba(251,146,60,.12)}36%{transform:translateX(0) scale(1);box-shadow:none}}
  @keyframes docPillEmitter{0%{opacity:0;transform:translate(-10px,-50%) scaleX(.3)}6%{opacity:1;transform:translate(-4px,-50%) scaleX(1)}30%{opacity:.7;transform:translate(12px,-50%) scaleX(.55)}58%,100%{opacity:0;transform:translate(24px,-50%) scaleX(.15)}}
  html.doc-loading .tree-item.active{animation:docPillRecoil 1.1s cubic-bezier(.4,0,.2,1) infinite;transform-origin:left center;will-change:transform,box-shadow}
  ...
  html.doc-loading .tree-item.active::after{content:"";position:absolute;left:0;top:50%;width:20px;height:70%;border-radius:999px;background:linear-gradient(90deg,rgba(251,146,60,.95),rgba(254,215,170,.5),rgba(251,146,60,0));filter:blur(.2px);animation:docPillEmitter 1.1s cubic-bezier(.4,0,.2,1) infinite;z-index:0;pointer-events:none}
  ...
  .theme-icon-window{width:20px;height:20px;overflow:hidden;display:inline-flex;align-items:flex-start;justify-content:center}
...
(function(){
  ...
  var themeMedia=matchMedia("(prefers-color-scheme:dark)");
  var themeToggle=document.getElementById("theme-toggle");
  var themeIconTrack=themeToggle&&themeToggle.querySelector(".theme-icon-track");
  ...
  function setThemeIconPosition(position,animate){
    if(!themeIconTrack)return;
    if(themeIconResetTimer)window.clearTimeout(themeIconResetTimer);
    if(!animate||matchMedia("(prefers-reduced-motion: reduce)").matches){
      themeIconTrack.style.transition="none";
      themeIconTrack.style.transform="translateY(-"+(position*20)+"px)";
      void themeIconTrack.offsetHeight;
      themeIconTrack.style.transition="";
      return;
    }
    themeIconTrack.style.transform="translateY(-"+(position*20)+"px)";
  }
  function resetThemeIconToSystem(){
    if(!themeIconTrack)return;
    themeIconTrack.style.transition="none";
    themeIconTrack.style.transform="translateY(0)";
    void themeIconTrack.offsetHeight;
    themeIconTrack.style.transition="";
  }
  function renderThemeButton(mode,animate){
    if(!themeToggle||!themeIconTrack)return;
    var current=themeToggle.dataset.themeMode||mode;

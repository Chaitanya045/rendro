...
function pageBody(page: AuthPage, user?: User, verified = false, token = ""): string {
  ...
  if (page === "verify-email") {
    return `<div class="heading status-heading"><span class="status-icon ${verified ? "success" : "mail"}"><span class="material-symbols-outlined" aria-hidden="true">${verified ? "mark_email_read" : "mail"}</span></span><p class="eyebrow">Email verification</p><h1>${verified ? "Email verified" : "Verify your email"}</h1><p>${verified ? "Your account is ready. Sign in to continue." : "Open the verification link we sent to your work email. The link expires after one hour."}</p></div>
  ...
  if (page === "forgot-password") {
    return `<div class="heading status-heading"><span class="status-icon mail"><span class="material-symbols-outlined" aria-hidden="true">key</span></span><p class="eyebrow">Account recovery</p><h1>Reset your password</h1><p>Enter your work email. If an account exists, we will send a secure reset link.</p></div>
  ...
  return `<div class="heading"><p class="eyebrow">Account security</p><h1>Sign-in methods</h1><p>Manage the ways you sign in as <strong>${escapeHtml(user?.email ?? "")}</strong>.</p></div>
    <div class="method-list" id="method-list" aria-live="polite" aria-busy="true"><div class="method method-skeleton" aria-hidden="true"><span class="method-skeleton-icon"></span><span class="method-skeleton-copy"><span></span><span></span></span><span class="method-skeleton-pill"></span></div><div class="method method-skeleton" aria-hidden="true"><span class="method-skeleton-icon"></span><span class="method-skeleton-copy"><span></span><span></span></span><span class="method-skeleton-pill"></span></div><span class="sr-only">Loading sign-in methods</span></div>
    ...
    <p class="message" id="security-message" role="status"></p><div class="account-footer"><a href="/">Back to workspace</a><a href="/api/auth/sign-out">Sign out</a></div>`;
}

const authRefreshStyles = `
.topbar{height:56px;padding:0 clamp(18px,4vw,44px);position:relative;z-index:2}.button:focus-visible,.reveal:focus-visible,.method-action:focus-visible{outline:2px solid var(--accent);outline-offset:2px}
...
.card{width:100%;max-width:440px;padding:32px;border:1px solid var(--border);border-radius:12px;box-shadow:0 18px 48px rgba(24,24,27,.07)}.card.wide{max-width:620px}.heading h1{letter-spacing:-.035em}.button{transition:background 150ms cubic-bezier(.4,0,.2,1),border-color 150ms cubic-bezier(.4,0,.2,1),color 150ms cubic-bezier(.4,0,.2,1),transform 150ms cubic-bezier(.4,0,.2,1),opacity 150ms cubic-bezier(.4,0,.2,1)}.button:active:not(:disabled):not([aria-disabled=true]){transform:scale(.98)}.button:disabled,.button[aria-disabled=true]{cursor:not-allowed;opacity:.62;transform:none}.button:hover:disabled,.button:hover[aria-disabled=true]{transform:none}.primary:hover:not(:disabled):not([aria-disabled=true]){background:var(--accent-hover);border-color:var(--accent-hover)}.primary:hover:disabled,.primary:hover[aria-disabled=true]{background:var(--accent);border-color:var(--accent)}.secondary:hover:not(:disabled):not([aria-disabled=true]),.google:hover:not(:disabled):not([aria-disabled=true]){background:var(--surface-2)}.secondary:hover:disabled,.secondary:hover[aria-disabled=true],.google:hover:disabled,.google:hover[aria-disabled=true]{background:var(--surface);border-color:var(--border)}.field label{display:block;font-size:12px;font-weight:650;margin-bottom:7px}.field>.input-wrap{font-size:inherit;font-weight:inherit;margin-bottom:0}.field input{transition:border-color 150ms cubic-bezier(.4,0,.2,1),box-shadow 150ms cubic-bezier(.4,0,.2,1),background 150ms cubic-bezier(.4,0,.2,1)}.field input:focus-visible{border-color:var(--accent);box-shadow:0 0 0 3px var(--focus)}.field input:disabled{cursor:not-allowed;opacity:.62;background:var(--surface-2)}.field input[aria-invalid=true]{border-color:var(--danger)}.reveal{top:0;right:0;height:44px;min-width:52px;border:1px solid transparent;border-radius:6px;transition:background 150ms cubic-bezier(.4,0,.2,1),border-color 150ms cubic-bezier(.4,0,.2,1),color 150ms cubic-bezier(.4,0,.2,1),transform 150ms cubic-bezier(.4,0,.2,1),opacity 150ms cubic-bezier(.4,0,.2,1)}.reveal:hover:not(:disabled){background:var(--surface-2);border-color:var(--border);color:var(--text)}.reveal:active:not(:disabled){transform:scale(.98)}.reveal:disabled{cursor:not-allowed;opacity:.62}.method-action{min-height:36px;display:inline-flex;align-items:center;justify-content:center;transition:background 150ms cubic-bezier(.4,0,.2,1),border-color 150ms cubic-bezier(.4,0,.2,1),color 150ms cubic-bezier(.4,0,.2,1),transform 150ms cubic-bezier(.4,0,.2,1),opacity 150ms cubic-bezier(.4,0,.2,1)}.method-action:hover:not(:disabled){border-color:var(--muted);background:var(--surface-2)}.method-action:active:not(:disabled){transform:scale(.98)}.method-action:disabled{cursor:wait;opacity:.62;transform:none}.auth-context-mobile{display:none;margin:0 0 20px;padding:12px 13px;border-left:3px solid var(--accent);background:color-mix(in srgb,var(--accent) 7%,var(--surface));color:var(--muted);font-size:13px}.auth-context-mobile strong{display:block;color:var(--text);margin-bottom:2px}.footer{font-size:11px}
@media(max-width:820px){.main{grid-template-columns:1fr;max-width:500px;padding-top:38px}.main.security-main{place-items:start center}.auth-context{display:none}.auth-context-mobile{display:block}.card,.card.wide{max-width:none}}
 .brand{display:inline-flex;align-items:center;min-height:44px}.account-footer a,.switch a,.form-row>a{transition:color 150ms cubic-bezier(.4,0,.2,1)}@media(hover:hover){.account-footer a:hover,.switch a:hover,.form-row>a:hover{color:var(--accent)}}.account-footer a:focus-visible,.switch a:focus-visible,.form-row>a:focus-visible,.brand:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
 .field input:hover:not(:disabled):not(:focus):not([aria-invalid=true]){border-color:var(--muted)}
@media(max-width:760px){.method-action{min-height:44px}.account-footer a{min-height:44px;display:inline-flex;align-items:center}}
@media(max-width:520px){.main{padding:24px 12px}.card{padding:24px 20px;border-radius:10px}.topbar{height:56px}.heading h1{font-size:26px}.form-row{align-items:center;min-height:44px}.form-row .check,.form-row>a{min-height:44px;display:inline-flex;align-items:center}}
@media(prefers-reduced-motion:reduce){.button,.field input,.method-action{transition-duration:.01ms}.reveal,.account-footer a,.switch a,.form-row>a{transition-duration:.01ms}.button:active:not(:disabled),.method-action:active:not(:disabled){transform:none}.button:active:not(:disabled):not([aria-disabled=true]),.method-action:active:not(:disabled):not([aria-disabled=true]),.reveal:active:not(:disabled){transform:none!important}.method-skeleton-icon:after,.method-skeleton-copy span:after,.method-skeleton-pill:after{animation:none}}
@media(prefers-reduced-motion:reduce){.button,.field input,.method-action,.reveal,.account-footer a,.switch a,.form-row>a{transition-duration:.01ms}.field input,.method-action{transition-duration:.01ms}}
`;
...
function renderAuthPage(
  ...
  const isInvitation = returnTo.startsWith("/accept-invitation/");
  const context = isInvitation
    ? `<aside class="auth-context invitation-context"><span class="invitation-chip">Organization invitation</span><h2>You have been invited to collaborate.</h2><p>Sign in with the exact invited email. Rendro preserves the invitation through authentication, then shows the organization and role before you accept.</p></aside>`
    : page === "security"
      ? `<aside class="auth-context"><p class="eyebrow">Account security</p><h2>Keep access deliberate.</h2><p>Manage verified sign-in methods without changing organization memberships, project scopes, or deployed documentation.</p></aside>`
      : `<aside class="auth-context"><p class="eyebrow">Documentation operations</p><h2>Ship the docs your product deserves.</h2><p>Rendro connects source-controlled documentation to immutable deployments, deliberate publication, and organization-scoped access.</p><div class="context-list"><span class="context-item">Deploy from the CLI with project-scoped credentials</span><span class="context-item">Keep releases immutable and publication explicit</span><span class="context-item">Invite people without weakening tenant boundaries</span></div></aside>`;
  const mobileContext = isInvitation
    ? `<div class="auth-context-mobile"><strong>Organization invitation</strong>Sign in with the exact email address that received the invitation.</div>`
    : "";
  return `<!DOCTYPE html>

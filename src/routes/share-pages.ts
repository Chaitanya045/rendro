...
(function(){
  "use strict";
  var ui=window.RendroUI,state=window.__RENDRO_PAGE_STATE__,list=document.getElementById("share-list"),query="?organizationId="+encodeURIComponent(state.organizationId)+"&projectId="+encodeURIComponent(state.projectId);
  ...
  async function load(){var results=await Promise.all([ui.request("/api/rendro/shares"+query),ui.request("/api/rendro/projects/get"+query),ui.request("/api/auth/organization/get-full-organization?organizationId="+encodeURIComponent(state.organizationId))]),project=results[1].project,organization=results[2];document.querySelectorAll("[data-org-name]").forEach(function(node){node.textContent=organization.name;});document.querySelectorAll("[data-org-mark]").forEach(function(node){node.textContent=organization.name.charAt(0).toUpperCase();});document.querySelector(".cp-page-description").textContent="Revocable, expiring links pinned to immutable deployments in "+project.name+".";render(results[0].shares);}
  load().catch(function(error){list.innerHTML='<tr><td colspan="5"><div class="empty compact"><span class="empty-mark">!</span><h2>Unable to load private shares</h2><p></p></div></td></tr>';list.querySelector("p").textContent=error.message;});
})();
`;

...
(function(){
  ...
  function setAccess(organization){var member=(organization.members||[]).find(function(candidate){return candidate.userId===state.userId;}),roles=member?String(member.role||"").split(",").map(function(role){return role.trim();}):[];canManage=roles.some(function(role){return role==="owner"||role==="admin";});if(!active())return;document.getElementById("share-access-note").hidden=canManage;}

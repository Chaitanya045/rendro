...
(function(){
  "use strict";
  var ui=window.RendroUI,state=window.__RENDRO_PAGE_STATE__,orgId=state.organizationId,mount=document.getElementById("organization-content"),loadVersion=0,dataPaths=${JSON.stringify(organizationSectionDataPaths(section))};
  ...
  async function baseData(){await post("/api/auth/organization/set-active",{organizationId:orgId});var query="?organizationId="+encodeURIComponent(orgId),results=await Promise.all(dataPaths.map(function(path){return ui.request(path+query);})),organization=results[0],invitations=[],projects=[];if(state.section==="overview"){invitations=results[1]||[];projects=results[2].projects||[];}else if(state.section==="people")invitations=results[1]||[];if(!organization)throw new Error("Organization not found.");setOrganizationChrome(organization);return {organization:organization,invitations:invitations,projects:projects};}
  function statusEmpty(mark,title,copy,action){return '<section class="panel empty"><span class="empty-mark material-symbols-outlined" aria-hidden="true">'+mark+'</span><h2>'+title+'</h2><p>'+copy+'</p>'+action+'</section>';}
  function watchCollectionEmpties(){if(state.section==="people"){var invitations=document.getElementById("invitation-list");new MutationObserver(function(){if(!invitations.children.length)invitations.innerHTML='<tr><td colspan="4"><div class="mini-empty">No pending invitations.</div></td></tr>';}).observe(invitations,{childList:true});}if(state.section==="teams"){var grid=document.getElementById("team-grid");new MutationObserver(function(){if(!grid.querySelector(".team-card")&&!grid.querySelector(".empty")){grid.innerHTML=statusEmpty("groups","No teams yet","Create a team, then assign existing organization members.",'<button class="button primary" type="button">Create first team</button>');grid.querySelector("button").addEventListener("click",function(){ui.openDialog("team-dialog");});}}).observe(grid,{childList:true});}}
...
function renderOrganizationSection(user: User, organizationId: string, section: OrganizationPageSection): string {
  ...
  return renderControlPlanePage({
    ...
    script: organizationPageScript(section),
  });
}

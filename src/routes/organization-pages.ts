...
import { renderControlPlanePage, type ControlPlaneSection } from "./control-plane";
...
(function(){
  "use strict";
  var ui=window.RendroUI,state=window.__RENDRO_PAGE_STATE__,mount=document.getElementById("organization-state");
  ...
  async function load(){try{var organizations=await ui.request("/api/auth/organization/list"),choose=new URLSearchParams(location.search).get("choose")==="1";if(!organizations.length){creationForm(true);return;}if(organizations.length===1&&!choose){location.replace("/organizations/"+encodeURIComponent(organizations[0].id));return;}mount.innerHTML='<div class="organization-grid" id="organization-list"></div><div class="chooser-footer"><button class="button" id="show-create" type="button">New organization</button></div>';var list=document.getElementById("organization-list");organizations.forEach(function(organization){list.appendChild(organizationCard(organization));});document.getElementById("show-create").addEventListener("click",function(){creationForm(false);});}catch(error){mount.innerHTML='<section class="panel empty"><span class="empty-mark">!</span><h2>Unable to load organizations</h2><p></p><button class="button" type="button">Try again</button></section>';mount.querySelector("p").textContent=error.message;mount.querySelector("button").addEventListener("click",load);}}
  load();
})();
`;
...
function renderOrganizationsPage(user: User): string {
  return renderControlPlanePage({
    ...
    focused: true,
    content: `<div id="organization-state"><div class="panel skeleton loading-panel" aria-label="Loading organizations"></div></div><style>.organization-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.organization-card{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:12px;text-decoration:none;transition:border-color var(--cp-instant) var(--cp-ease),transform var(--cp-instant) var(--cp-ease),background var(--cp-instant) var(--cp-ease)}.organization-card:hover{border-color:var(--cp-accent);background:var(--cp-accent-soft);transform:translateY(-1px)}.organization-arrow{color:var(--cp-accent);font-size:12px;font-weight:750}.chooser-footer{display:flex;justify-content:center;margin-top:18px}@media(max-width:620px){.organization-grid{grid-template-columns:1fr}}</style>`,
    script: organizationChooserScript,
  });
}

function sectionHeading(section: OrganizationPageSection): { active: ControlPlaneSection; eyebrow: string; heading: string; description: string; actions: string } {
  if (section === "people") return {
    ...
    actions: '<button class="button primary" type="button" data-dialog-open="invite-dialog">Invite people</button>',
  };
  if (section === "teams") return {
    ...
    actions: '<button class="button primary" type="button" data-dialog-open="team-dialog">Create team</button>',
  };
  ...
  return {
    ...
    actions: '<a class="button" data-project-link href="#">View projects</a><a class="button primary" data-onboarding-link href="#">Create project</a>',
  };
}
...
(function(){
  "use strict";
  var ui=window.RendroUI,state=window.__RENDRO_PAGE_STATE__,orgId=state.organizationId,mount=document.getElementById("organization-content");
  ...
  function setOrganizationChrome(organization){document.querySelectorAll("[data-org-name]").forEach(function(node){node.textContent=organization.name;});document.querySelectorAll("[data-org-mark]").forEach(function(node){node.textContent=(organization.name||"R").charAt(0).toUpperCase();});document.querySelectorAll("[data-project-link]").forEach(function(link){link.href="/organizations/"+encodeURIComponent(orgId)+"/projects";});document.querySelectorAll("[data-onboarding-link]").forEach(function(link){link.href="/organizations/"+encodeURIComponent(orgId)+"/onboarding";});}
  ...
  async function load(){try{var data=await baseData();if(state.section==="people")await renderPeople(data);else if(state.section==="teams")await renderTeams(data);else if(state.section==="settings")await renderSettings(data);else await renderOverview(data);}catch(error){mount.innerHTML=statusEmpty("!","Unable to load this organization",error.message,'<button class="button" type="button" id="retry">Try again</button>');document.getElementById("retry").addEventListener("click",load);}}
  load();
})();
`;
...
(function(){
  "use strict";
  var ui=window.RendroUI,state=window.__RENDRO_PAGE_STATE__,orgId=state.organizationId,projectId=state.projectId,mount=document.getElementById("onboarding-stage"),pollTimer=null;
  function slugify(value){return value.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"").slice(0,48);}
...
(function(){
  ...
  async function respond(action,button){ui.busy(button,true);try{var result=await ui.request("/api/auth/organization/"+action,{method:"POST",body:JSON.stringify({invitationId:state.invitationId})});if(action==="accept-invitation"){location.assign("/organizations/"+encodeURIComponent(result.member.organizationId));}else location.assign("/organizations");}catch(error){document.getElementById("invitation-error").textContent=error.message;ui.busy(button,false);}}
  load();
})();
`;
